...
Progress: the Squad workspace now uses a responsive split command view with the subgroup roster and collapsed coverage disclosures on the left, plus a searchable saved-build roster and two-column tactical matrix on the right. Five-slot subgroup scanning, Build Viewer activation, coverage calculations, and squad persistence remain unchanged. Populated desktop, tablet, and phone layouts have been visually checked with assigned and open slots. The roster steps from five columns to three, two, and one as space narrows, and the checked widths have no horizontal overflow or duplicated empty-slot decoration. Saved builds can be copied from the library, while existing assignments can be moved or swapped between subgroup slots through either drag-and-drop or a focus-trapped keyboard dialog. Live checks confirm picker and move-dialog focus containment, Escape restoration, assignment, movement, swapping, removal, and focus handoff between slot states. Phase C acceptance is complete.
...
Progress: the focused Build Viewer already handles focus trapping and restoration, browser Back, Escape, body scroll locking, reduced motion, and responsive modal-to-sheet behavior. Both searchable Builder picker variants now match those modal fundamentals by trapping forward/reverse tab movement, closing on Escape or backdrop interaction, restoring trigger focus, and locking background scroll. Live interaction checks additionally verify slot-picker focus containment and focus continuity after keyboard assignment and removal. Empty and populated Builder states have been checked at 320, 390, 768, and desktop widths with no root or document-level horizontal overflow; populated Library and Squad layouts now hold their intended hierarchy at the checked widths. Uncached image checks found no incomplete, broken, or zero-sized Builder images, with zero measured phone layout shift and low desktop startup shift attributable to shell sizing rather than images. Viewer open/close sampling under normal and reduced-motion preferences found no geometry-driven layout shifts, and long spaced or unbroken build names remain contained in Library cards and the 320/390px viewer header. A 25-player stress fixture rendered all five subgroups without overflow at desktop, tablet, and phone widths; initial populated rendering completed well under two seconds in the development build. A source audit found no icon-only Builder button, link, or summary without an accessible name or title. An automated contrast scan found no definite violation, but 25 layered/gradient-backed elements remained indeterminate and still require manual confirmation. Broader interactive focus-order verification and that manual contrast confirmation remain.
