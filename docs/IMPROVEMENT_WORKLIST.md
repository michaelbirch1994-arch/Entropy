...
- [ ] Replace percentage-first Buff Generation values with total generated seconds so this page answers a different question from Buff Uptime and Party Boons.
  - Show total boon-duration seconds generated by each player for each boon where Elite Insights provides generation duration.
  - Prefer a duration format such as `2m 34s` for readability while preserving the raw-second value for sorting/tooltips.
  - Keep percentage uptime on Buffs and Party Boons rather than repeating the same representation here.
- [ ] Add wasted/overgenerated boon-duration seconds only where the underlying EI field can be validated as true wasted duration.
  - Do not relabel `overstack_value` as wasted seconds until its meaning and unit are independently confirmed.
  - If exact waste cannot be derived safely, show generated seconds only and leave waste explicitly unavailable rather than inventing it.
- [ ] Verify generation totals across multiple fights, subgroup changes, and partial attendance, and make sorting use the displayed duration values.

## P2 — Fight Replay inspection

- [ ] Add paused-replay hover inspection for allied player markers.
  - Only show the detailed hover card when playback is paused so the UI remains usable during motion.
  - Identify the player by name and profession/specialization at the selected replay timestamp.
  - Show boons active at that timestamp, including stack count or remaining duration where the parsed replay/buff timeline supports it.
  - Show conditions active at that timestamp using the same evidence rules as boons.
  - Show whether the player is currently crowd-controlled and, where derivable, the CC state/type.
  - Keep the tooltip anchored to the hovered marker without obscuring nearby replay dots or controls.
- [ ] Reuse timestamped EI combat-state/buff data rather than estimating state from aggregate uptime percentages.
- [ ] Gracefully mark state as unavailable for archived reports or logs that do not persist the timestamp-level data required for the hover card.
- [ ] Add replay regression coverage for pause/hover state, timestamp boundaries, missing buff-state data, stacked boons/conditions, and CC transitions.
