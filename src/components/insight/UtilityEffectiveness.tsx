  ...
  return <section className="utility-effectiveness" aria-label="Utility effectiveness chart">
    ...
    <div className="utility-scoreboard" data-mode={mode}>
      {mode === 'stability' ? <>
        ...
        <div><ShieldCheck size={18}/><span><small>{stabilityIsEstimated ? 'STATE READINESS' : 'THREAT READINESS'}</small><strong>{percent(model.overall.stability.readinessRating)}</strong><em>{model.overall.stability.protectedAttempts}/{model.overall.stability.threatAttempts} observed events met with Stability</em></span></div>
        ...
        <div><ShieldCheck size={18}/><span><small>ARRIVAL READINESS</small><strong>{percent(resistanceSummary.readinessRating)}</strong><em>{resistanceSummary.coveredArrivals}/{resistanceSummary.conditionArrivals} {activeResistanceCondition ?? 'condition'} arrivals met with Resistance already active</em></span></div>
    ...
    {mode === 'resistance' && model.overall.resistance.conditions.length ? <section className="utility-condition-lens" aria-label="Resistance condition lens">
      ...
      <div role="list">
        <button type="button" role="listitem" aria-pressed={!activeResistanceCondition} onClick={() => setSelectedResistanceCondition(null)}>
          <span>All pressure</span><strong>{percent(model.overall.resistance.effectiveRating)}</strong><small>{seconds(model.overall.resistance.suppressedConditionMs / 1000, 1)} suppressed · {percent(model.overall.resistance.readinessRating)} ready</small>
        </button>
        {model.overall.resistance.conditions.map(condition => <button type="button" role="listitem" key={condition.condition} aria-pressed={activeResistanceCondition === condition.condition} onClick={() => setSelectedResistanceCondition(condition.condition)}>
          <span>{condition.condition}</span><strong>{percent(condition.effectiveRating)}</strong><small>{seconds(condition.suppressedConditionMs / 1000, 1)} / {seconds(condition.conditionPressureMs / 1000, 1)} · {condition.coveredArrivals}/{condition.conditionArrivals} arrivals</small>
        </button>)}
      </div>
        ...
        {[...model.subgroups, model.overall].map(scope => {
          ...
          return <article role="listitem" key={scope.key} data-overall={scope.group === null}>
            ...
            <small>{mode === 'stability'
              ? `${percent(scope.stability.readinessRating)} readiness · ${scope.stability.successfulControls} controls landed · ${percent(scope.stability.evidenceCoverage)} ${stabilityIsEstimated ? 'timeline coverage' : 'evidence'}`
              : mode === 'resistance'
                ? `${percent(scopedResistance?.readinessRating ?? null)} arrival readiness · ${scopedResistance?.coveredArrivals ?? 0}/${scopedResistance?.conditionArrivals ?? 0} arrivals covered · ${activeResistanceCondition ? scopedResistanceCondition?.affectedPlayers ?? 0 : scope.resistance.conditionTrackedPlayers} players`
              : mode === 'aegis' ? (aegisIsEstimated
                ...
                : `${scope.aegis.confirmedBlocks} confirmed blocks · ${percent(scope.aegis.readinessRating)} attack readiness · ${scope.aegis.landedWhileAegisPresent} uncertain hits`)
      ...
      {mode === 'stability' ? <div className="utility-chart" role="table" aria-label="Stability effectiveness by player">
        ...
        <div className="utility-chart-rows">
          {model.stability.length ? model.stability.map(row => <button type="button" role="row" aria-selected={selectedStability?.account === row.account} key={row.account} onClick={() => setSelectedProvider(current => ({ ...current, stability: row.account }))}>
            ...
            <span role="cell" data-label={stabilityIsEstimated ? 'State readiness' : 'Threat readiness'}><strong>{percent(row.readinessRating)}</strong><small>{row.protectedAttempts}/{row.threatAttempts} events protected · {row.successfulControls} landed</small></span>
        ...
        <div className="utility-chart-rows">
          {resistanceRows.length ? resistanceRows.map(row => {
            ...
            return <button type="button" role="row" aria-selected={selectedResistance?.account === row.account} key={row.account} onClick={() => setSelectedProvider(current => ({ ...current, resistance: row.account }))}>
              ...
              <span role="cell" data-label="Arrival readiness"><strong>{percent(metric.readinessRating)}</strong><small>{metric.coveredArrivals}/{metric.conditionArrivals} arrivals covered · {row.pressureAlignedCasts}/{row.casts} casts aligned</small></span>
        ...
        <div className="utility-chart-rows">
          {model.aegis.length ? model.aegis.map(row => <button type="button" role="row" aria-selected={selectedAegis?.account === row.account} key={row.account} onClick={() => setSelectedProvider(current => ({ ...current, aegis: row.account }))}>
            ...
            <span role="cell" data-label={aegisIsEstimated ? 'Evidence basis' : 'Attack readiness'}><strong>{aegisIsEstimated ? percent(model.overall.aegis.evidenceCoverage) : percent(row.readinessRating)}</strong><small>{aegisIsEstimated ? 'Player Aegis timeline coverage' : `${row.readyAttempts}/${row.threatAttempts} attacks · ${row.landedWhileAegisPresent} uncertain hits`}</small></span>
        ...
        <div className="utility-event-list">
          {selectedResistanceApplications.length ? selectedResistanceApplications.map(application => {
            ...
            return <button type="button" key={application.key} onClick={() => onMoment(selectedResistance.account, application.skillId, application.timeMs)}>
              ...
              <span><small>{timestamp(application.timeMs)} · {seconds((application.endMs - application.timeMs) / 1000, 1)} reference window</small><strong>{application.skillName}</strong><em>{activeResistanceCondition ? `${metric.coveredArrivals}/${metric.conditionArrivals} ${activeResistanceCondition} arrivals covered` : application.conditionNames.length ? `${application.conditionNames.join(', ')} · ${application.coveredArrivals}/${application.conditionArrivals} arrivals covered` : 'No observed non-damaging condition pressure in this window'}</em></span>
    ...
    <footer className="utility-effectiveness-footer">
      <span>{model.fightName}</span><span>{model.coverage.stabilityTrackedPlayers} Stability · {model.coverage.resistanceTrackedPlayers} Resistance · {model.coverage.aegisTrackedPlayers} Aegis timelines / {model.coverage.replayRosterPlayers} players</span><span>{loading ? 'Loading ArenaNet references' : references?.failed ? 'API references unavailable; wiki WvW references retained' : `${currentReferences.length}/${model.coverage.observedFightSkills} fight skills referenced`}</span>
    </footer>
