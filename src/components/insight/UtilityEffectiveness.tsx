...
export default function UtilityEffectiveness({ report, fightId, onPlayer, onMoment }: {
  ...
  const timelineCoverage = model.coverage.effectTimelineCoverage;
  ...
  return <section className="utility-effectiveness" aria-label="Utility effectiveness chart">
    ...
    <footer className="utility-effectiveness-footer">
      <span>{model.fightName}</span><span>{model.coverage.stabilityTrackedPlayers} Stability · {model.coverage.resistanceTrackedPlayers} Resistance · {model.coverage.aegisTrackedPlayers} Aegis tracks</span><span>Raw timelines {timelineCoverage.available}/{timelineCoverage.rosterPlayers} · {timelineCoverage.partial} partial · {timelineCoverage.legacyUnknown} legacy</span><span>{model.subgroupEvidence.source === 'fight-replay' ? 'Selected-fight parties' : model.subgroupEvidence.source === 'mixed' ? 'Mixed party evidence' : 'Aggregate party fallback'}</span><span>{loading ? 'Loading ArenaNet references' : references?.failed ? 'API references unavailable; wiki WvW references retained' : `${currentReferences.length}/${model.coverage.observedFightSkills} fight skills referenced`}</span>
    </footer>
