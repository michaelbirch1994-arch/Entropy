      ...
      {mechanics.length > 0 && (
        <section className="mt-4 rounded-2xl border border-theme-border bg-black/20 p-4" aria-label="Replay mechanics in selected Intelligence window">
          ...
          <div className="mt-4 grid gap-3 xl:grid-cols-3">
            ...
            <div className="rounded-2xl border border-theme-focus bg-theme-accentDim p-3"><div className="flex items-center justify-between gap-2"><span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-theme-accentStrong"><Clock3 className="h-3 w-3" /> At anchor</span><span className="font-mono text-[10px] text-slate-600">{anchor.length}</span></div><div className="mt-2 grid gap-2">{anchor.length ? anchor.map((mechanic, index) => <MechanicCard key={`${mechanic.timestampMs}-${mechanic.name}-${index}`} mechanic={mechanic} onOpenReplay={() => openReplay(mechanic)} />) : <p className="text-[11px] leading-5 text-slate-600">No mechanic marker occurred at the exact Intelligence anchor timestamp.</p>}</div></div>
            <div className="rounded-2xl border border-white/[0.06] bg-black/20 p-3"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-black uppercase tracking-wider text-slate-400">After</span><span className="font-mono text-[10px] text-slate-600">{after.length}</span></div><div className="mt-2 grid gap-2">{after.length ? after.map((mechanic, index) => <MechanicCard key={`${mechanic.timestampMs}-${mechanic.name}-${index}`} mechanic={mechanic} onOpenReplay={() => openReplay(mechanic)} />) : <p className="text-[11px] leading-5 text-slate-600">No replay mechanic markers after the anchor inside this window.</p>}</div></div>
          </div>
        </section>
      )}
    </>
