          ...
          <div className="mt-3 grid gap-3">
            {deathEvidence.map(({ recap, linkedPlayer, offsetMs }) => (
              <details key={`${recap.fightIndex}-${recap.account}-${recap.deathTimeMs}`} className="group rounded-xl border border-white/[0.06] bg-black/25 p-3">
                <summary className="cursor-pointer list-none"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-sm font-black text-slate-100">{recap.account}</div><div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500"><span>{recap.characterName}</span><ProfessionIdentity profession={recap.profession} size="sm" /></div></div><div className="flex flex-wrap items-center gap-2">{linkedPlayer && <span className="rounded-full border border-theme-focus bg-theme-accentDim px-2 py-1 text-[10px] font-bold uppercase text-theme-accentStrong">linked player</span>}<span className="rounded-full border border-white/10 bg-white/[0.03] px-2 py-1 font-mono text-[10px] font-bold text-slate-400">{formatTime(recap.deathTimeMs)} · {formatOffset(offsetMs)}</span></div></div></summary>
