      ...
      {fight && (
        <Panel title="Fight Replay" subtitle={`${fight.fightName} · tactical movement + exact-time player state`} icon={<Film className="h-3.5 w-3.5" />} action={`${fight.data.players.length} players tracked`} className={focusMode ? "rounded-xl" : ""} bodyClassName={focusMode ? "p-3" : "p-5"}>
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {zoom > 1 && <button type="button" aria-pressed={followFocus} onClick={() => { setFollowFocus((value) => !value); setPan({ x: 0, y: 0 }); }} className={`rounded border px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${followFocus ? "border-sky-500/30 bg-sky-500/10 text-sky-300" : "border-slate-800 text-slate-500"}`}>Follow focus</button>}
