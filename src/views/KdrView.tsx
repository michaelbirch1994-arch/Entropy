  ...
  return (
    <div className="space-y-6 animate-view pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Panel title="Squad Trade Record" subtitle="Canonical KDR: enemy deaths credited by squad divided by allied deaths." icon={<Swords className="w-4 h-4" />} accent="text-emerald-400">
          <div className="space-y-4">
            ...
            <KdrBar label="Enemy downs" value={s.totalSquadDowns} max={Math.max(s.totalSquadDowns, s.totalEnemyDowns)} color="bg-sky-500" />
            <KdrBar label="Allied deaths" value={s.totalSquadDeaths} max={Math.max(s.totalSquadKills, s.totalSquadDeaths)} color="bg-rose-500" />
          </div>
        </Panel>

        <Panel title="Pressure Against Squad" subtitle="This is not a second KDR formula; it shows the enemy pressure that produced allied downs and deaths." icon={<Skull className="w-4 h-4" />} accent="text-rose-400">
          <div className="space-y-4">
            <KdrBar label="Allied deaths" value={s.totalSquadDeaths} max={Math.max(s.totalSquadKills, s.totalSquadDeaths)} color="bg-rose-500" />
      ...
      <Panel title="Fight-by-Fight Outcome" icon={<Crosshair className="w-4 h-4" />} accent="text-sky-400">
        ...
        <div className="overflow-x-auto custom-scrollbar rounded-xl border border-slate-800/50">
          <table className="w-full min-w-[760px] text-left border-collapse text-xs whitespace-nowrap">
            <thead>
              <tr className="text-[10px] text-slate-500 uppercase font-bold tracking-wider border-b border-slate-800/50 bg-slate-950/40">
            ...
            <tbody className="divide-y divide-slate-800/30 font-mono">
              {sortedFights.map(({ fight: f, index }) => (
                <tr key={f.id} className="hover:bg-sky-950/20 transition-colors">
                  <td className="p-2.5 text-slate-400">F{index + 1}</td>
                  <td className="p-2.5 text-slate-300 font-semibold">{f.mapName}</td>
                  ...
                  <td className="p-2.5 text-right text-slate-300">{fmtNum(f.squadCount)}</td>
                  <td className="p-2.5 text-right text-slate-300">{fmtNum(f.enemyCount)}</td>
                  <td className="p-2.5 text-right text-emerald-400">{fmtNum(f.enemyDeaths)}</td>
                  <td className="p-2.5 text-right text-sky-400">{fmtNum(f.enemyDowns)}</td>
