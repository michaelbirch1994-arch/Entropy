...
export function FightSelector({ fights, selectedFightId, onSelect, totals }: { fights: FightContext[]; selectedFightId: string; onSelect: (fightId: string) => void; totals: { downs: number; deaths: number; findings: number; criticalEvents: number } }) {
  const selectedIndex = fights.findIndex((fight) => fight.id === selectedFightId);
  return (
    <section className="entropy-intel-scope" aria-label="Intelligence fight scope">
      <label><MapPinned size={18} aria-hidden="true" /><span>Fight scope</span>
        <select aria-label="Intelligence fight scope" value={selectedFightId} onChange={(event) => onSelect(event.target.value)}>
          <option value={ALL_FIGHTS_ID}>All fights · {totals.downs}D / {totals.deaths}X</option>
          {fights.map((fight) => <option key={fight.id} value={fight.id}>{fight.label} · {fight.name} · {fight.downs}D / {fight.deaths}X</option>)}
        </select>
      </label>
      <div className="entropy-intel-scope-step">
        <button type="button" className="entropy-icon-button" aria-label="Previous Intelligence fight" title="Previous fight" disabled={selectedIndex < 0} onClick={() => onSelect(selectedIndex === 0 ? ALL_FIGHTS_ID : fights[selectedIndex - 1].id)}><ChevronLeft size={16} /></button>
        <button type="button" className="entropy-icon-button" aria-label="Next Intelligence fight" title="Next fight" disabled={selectedIndex >= fights.length - 1} onClick={() => onSelect(fights[selectedIndex + 1].id)}><ChevronRight size={16} /></button>
      </div>
      <div className="entropy-intel-scope-totals"><span><strong>{fights.length || "unknown"}</strong> fights</span><span><strong>{totals.criticalEvents}</strong> events</span></div>
    </section>
  );
}
...
function StatCard({ icon, label, value, detail }: { icon: ReactNode; label: string; value: string; detail: string }) {
  return <div className="entropy-intel-stat"><div className="flex items-center justify-between gap-3"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">{label}</div><div className="text-amber-400">{icon}</div></div><div className="mt-3 text-2xl font-black text-slate-100">{value}</div><div className="mt-1 text-xs leading-5 text-slate-500">{detail}</div></div>;
}
...
export function CriticalEventCard({ event, fightContext, selected, onInspect }: { event: CriticalEvent; fightContext: FightContext; selected: boolean; onInspect: () => void }) {
  return (
    <button type="button" aria-pressed={selected} onClick={onInspect} className="entropy-intel-event">
      <span className="entropy-intel-event-clock">{formatTime(event.timestampMs)}<small>{fightContext.label}</small></span>
      <span className="entropy-intel-event-copy">
        <span className="entropy-intel-event-title"><strong>{event.kind}</strong><small>{event.confidence}</small></span>
        <span className="entropy-intel-event-summary">{event.summary}</span>
        <span className="entropy-intel-event-location">{fightContext.name}</span>
      </span>
      <span className="entropy-intel-event-open" title={selected ? "Inspecting event" : "Inspect event"}>{selected ? <Eye size={16} /> : <ChevronRight size={16} />}</span>
    </button>
  );
}
...
export default function IntelligenceDebugView() {
  ...
  const eventEvidenceRef = useRef<HTMLDivElement>(null);
  ...
  function inspectCriticalEvent(eventId: string, episodeId: string | null = null) {
    setSelectedCriticalEventId(eventId);
    setSelectedEpisodeId(episodeId);
    const evidence = eventEvidenceRef.current;
    if (!evidence) return;
    evidence.scrollTop = 0;
    if (episodeId !== null || window.matchMedia("(max-width: 1199px)").matches) {
  ...
  const externalReplayAvailable = externalEvidenceTarget
    ...
    : false;
  return (
    <div className="entropy-intelligence space-y-5 pb-12">
      <section className="entropy-intel-masthead">
        <div className="entropy-intel-masthead-copy">
          <div className="entropy-intel-eyebrow"><BrainCircuit size={18} /> Combat intelligence</div>
          <h2>Entropy Intelligence</h2><h3>{dashboard.headline}</h3><p>{dashboard.summary}</p>
          <div className="entropy-intel-masthead-status">
            <Pill className={`${readiness.classes} border-current/20`}>{readiness.icon}{readiness.label}</Pill>
            <Pill className={dashboard.persisted ? "border-emerald-400/20 bg-emerald-500/10 text-emerald-300" : "border-amber-400/20 bg-amber-500/10 text-amber-300"}>{dashboard.persisted ? "persisted data" : "legacy fallback"}</Pill>
            {selectedFight && <Pill>Viewing {selectedFight.label}</Pill>}
          </div>
        </div>
      </section>
      <nav className="entropy-intel-navigation" aria-label="Intelligence sections">
        <a href="#intelligence-events"><Radar size={15} /> Event workspace</a>
        {selectedFight && <a href="#intelligence-readout"><ListChecks size={15} /> Fight readout</a>}
        <a href="#intelligence-engagements"><Swords size={15} /> Engagements</a>
        <a href="#intelligence-analysis"><Layers3 size={15} /> Analysis</a>
      </nav>
      ...
      <section id="intelligence-events" className="theme-intelligence-heartbeat entropy-intel-events">
        <div className="entropy-intel-event-feed">
          ...
          <div className="mt-3 flex flex-wrap gap-2"><button type="button" aria-pressed={criticalEventKindFilter === "all"} onClick={() => { setCriticalEventKindFilter("all"); setExpandedSections((current) => ({ ...current, criticalEvents: false })); }} className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition ${criticalEventKindFilter === "all" ? "border-rose-300/40 bg-rose-400/[0.1] text-rose-200" : "border-white/10 bg-white/[0.03] text-slate-500 hover:text-slate-300"}`}>all {scopedCriticalEvents.length}</button>{criticalEventKinds.slice(0, 8).map((kind) => <button key={kind} type="button" aria-pressed={criticalEventKindFilter === kind} onClick={() => { setCriticalEventKindFilter(kind); setExpandedSections((current) => ({ ...current, criticalEvents: false })); }} className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition ${criticalEventKindFilter === kind ? "border-rose-300/40 bg-rose-400/[0.1] text-rose-200" : "border-white/10 bg-white/[0.03] text-slate-500 hover:text-slate-300"}`}>{kind} {criticalEventKindCounts.get(kind) ?? 0}</button>)}</div>
          <BoundedDataRegion label={`Critical event feed, ${visibleCriticalEvents.length} loaded events`} itemCount={visibleCriticalEvents.length} maxHeightClass="max-h-[660px]" className="mt-4 grid gap-2 pr-1">{visibleCriticalEvents.map((event) => <CriticalEventCard key={event.id} event={event} fightContext={fightContextFor(fightContexts, event.fightId)} selected={selectedCriticalEventId === event.id} onInspect={() => inspectCriticalEvent(event.id)} />)}{filteredCriticalEvents.length === 0 && <EmptyState dashboard={dashboard} scope={selectedScopeLabel} />}</BoundedDataRegion>
        ...
        <div className="entropy-intel-event-evidence">
      ...
      <section className="theme-intelligence-dossier border border-violet-400/15 bg-black/35 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">Episodes connect events already tied by a persisted engagement segment or shared player/event evidence. They are review groups, not causal claims; the complete event stream remains available.</p>
          </div>
