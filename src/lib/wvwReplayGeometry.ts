...
export interface WvWReplayGeometry extends Gw2MapGeometry {
  ...
  replayMapRect: [number, number, number, number];
  /** EI's normalized JSON combatReplayMetaData.sizes. */
  replayPixelSize: Point2;
}

/** Verified Elite Insights WvW replay geometry. */
...
/** Inverse of EI CombatReplayMap.GetMapCoordRounded. */
export function replayPixelToMapPoint(
  ...
  return [topX + nx * (bottomX - topX), topY + ny * (bottomY - topY)];
}

/** Mirrors EI CombatReplayMap.GetMapCoordRounded without output rounding. */
