...
export function eventResponses(report: WvWReport, fightId: string, event: CombatMoment, kind: ResponseKind) {
  ...
  return { event, kind, targetResolved: Boolean(target), rotationAvailable: Boolean(fight), candidates,
    ...
    referenceMode: reference.catalog
      ...
      : `Pinned reference catalog unavailable: ${report.meta?.referenceCatalog?.id ?? 'unknown'}`,
    ...
    limitations: [reference.catalog ? 'Supported recorded skills and user-defined Firebrand/Druid loadout assumptions are assessed. Other never-cast skills remain unknown.' : 'The report pins a reference catalog that this app version cannot resolve; response skills are not assessed.',
