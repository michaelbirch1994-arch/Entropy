...
old = "  const squadOverviewRows = buildSquadOverviewRows(s, scope, allyScope);\n"
new = """  const squadOverviewRows = (() => {
...
if text.count(old) != 1:
    raise SystemExit(f"Expected exactly one normalized roster rows line, found {text.count(old)}")
